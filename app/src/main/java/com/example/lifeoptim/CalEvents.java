...
public class CalEvents {
    ...
    public ArrayList<String> suggestions(ArrayList<String> locations) throws JSONException{
        ...
        weatherSuggestion.put("Overcast", "Chill");
        weatherSuggestion.put("Rainy", "Carry Umbrella");
        weatherSuggestion.put("Snow", "Carry Gloves");
        weatherSuggestion.put("High Winds", "Bring Jacket");
        String lat = "0.0";
        String lng = "0.0";
        int i;
        String temp = "empty";
        String extremeWeathers[] = {"Snow","High Winds","Rainy"};
        ArrayList<String> data = new ArrayList<>();
        ArrayList<String> suggestions = new ArrayList<>();
        int flag = 0;
        for(String location : locations) {
            flag =  0;
            data = WeatherAPI.get(lat,lng);
            for(i = 0 ; i < extremeWeathers.length; i++){
                for(String d : data){
                    if(d.equals(extremeWeathers[i])){
                        Log.d("%Extreme Weather", extremeWeathers[i] + " " + weatherSuggestion.get(extremeWeathers[i]));
                        temp = weatherSuggestion.getOrDefault(extremeWeathers[i],"yes");
                        Log.d("%%","" + temp);
